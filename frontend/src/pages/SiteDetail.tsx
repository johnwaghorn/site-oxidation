...
export function SiteDetail() {
    ...
    const { data: outagesData, isLoading: outagesLoading } = useOutages(siteId, page)

    const totalPages = outagesData ? Math.ceil(outagesData.total / outagesData.per_page) : 0
    ...
    return (
        <div style={{ maxWidth: '1200px', padding: '24px' }}>
            <Link to="/" style={{ color: '#6b7280', textDecoration: 'none', marginBottom: '16px', display: 'inline-block' }}>
            ...
            <section style={{ marginBottom: '32px' }}>
                ...
                <SiteForm
                    ...
                    mode="edit"
                    initialData={site}
                    onSubmit={payload => updateSite.mutate({ id: siteId, site: payload })}
                    isLoading={updateSite.isPending}
                />
                {updateSite.isError && <ErrorMessage error={updateSite.error} />}
                {updateSite.isSuccess && (
                    <p style={{ color: '#059669', margin: '8px 0' }}>Site updated successfully</p>
                )}
            </section>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
                                    ...
                                    <th style={{ padding: '12px 8px', textAlign: 'left' }}>Ended</th>
                                    <th style={{ padding: '12px 8px', textAlign: 'center' }}>HTTP Status</th>
                                    <th style={{ padding: '12px 8px', textAlign: 'left' }}>Error</th>
                                </tr>
                            </thead>
                            <tbody>
...
function OutageRow({ outage }: { outage: OutageResponse }) {
    ...
    const ended = outage.ended_at ? new Date(outage.ended_at).toLocaleString() : 'Ongoing'

    return (
        <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
            <td style={{ padding: '12px 8px' }}>{started}</td>
            <td style={{ padding: '12px 8px', color: outage.ended_at ? undefined : '#dc2626' }}>{ended}</td>
